...
# endif
  tmElements_t tm;
  digitalWrite(13, HIGH);     //Crude indicator to tell me that its running and reading time q3s
...
# if DEBUG == 1
    ...
    int mLeft = (mSunset - dayMinNow);
    Serial.println(mLeft); // Print Minutes Until Sunset
    int dayLeft = ((float(mLeft) / float(mDaylight)) * 100);
    ...
    */
    ...
    (((dayMinNow >= (mSunset - 10)) && (dayMinNow <= (mSunset + 90)))
        ||
        ((dayMinNow >= (mSunrise - 30)) && (dayMinNow <= (mSunrise + 90)))) {
      if (t > 85) {
